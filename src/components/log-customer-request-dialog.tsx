...
const formSchema = {
  ...
  companyRecord: Forms.attioRecord().objectSlug("companies"),
...
export function LogCustomerRequestDialog({
  ...
  const {
    ...
    companyRecord: companyRecordId
      ? { recordId: companyRecordId, objectSlug: "companies" }
      : undefined,
    description,
    addTo: "",
    title: "",
        ...
        try {
          if (values.addTo.startsWith("project")) {
            ...
            await createCustomerNeed({
              ...
              companyRecordId: values.companyRecord.recordId,
              projectId,
          ...
          if (values.companyRecord.recordId) {
            queryClient.invalidateQueries({
              queryKey: [
                "company-customer-request-count",
                values.companyRecord.recordId,
              ],
            });
          }
          onDone();
...
async function createCustomerNeed({
  ...
  const customerNeed = await createCustomerNeedServer({
    ...input,
